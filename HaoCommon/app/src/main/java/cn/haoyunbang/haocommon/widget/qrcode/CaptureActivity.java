...
import cn.haoyunbang.common.ui.base.BaseSwipeBackCompatActivity;
import cn.haoyunbang.common.util.BaseUtils;
import cn.haoyunbang.common.widget.log.LogUtil;
import cn.haoyunbang.haocommon.R;
import cn.haoyunbang.haocommon.widget.qrcode.camera.CameraManager;
import cn.haoyunbang.haocommon.widget.qrcode.decode.DecodeUtils;
import cn.haoyunbang.haocommon.widget.qrcode.utils.BeepManager;
import cn.haoyunbang.haocommon.widget.qrcode.utils.InactivityTimer;
...
//                readyGoForResult(CommonImagePickerListActivity.class, IMAGE_PICKER_REQUEST_CODE);
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        if (holder == null) {
            LogUtil.e(TAG_LOG, "*** WARNING *** surfaceCreated() gave us a null surface!");
        }
        if (!hasSurface) {
            hasSurface = true;
//            initCamera(holder);
    ...
    public void handleDecode(String result, Bundle bundle) {
        ...
        if (!BaseUtils.isEmpty(result) && BaseUtils.isUrl(result)) {
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        ...
        if (cameraManager.isOpen()) {
            LogUtil.w(TAG_LOG, "initCamera() while already open -- late SurfaceView callback?");
            return;
        }
        try {
            ...
            LogUtil.w(TAG_LOG, ioe);
            displayFrameworkBugMessageAndExit();
        } catch (RuntimeException e) {
            // Barcode Scanner has seen crashes in the wild of this variety:
            // java.?lang.?RuntimeException: Fail to connect to camera service
            LogUtil.w(TAG_LOG, "Unexpected error initializing camera", e);
            displayFrameworkBugMessageAndExit();
        }
...
//
//            if (!BaseUtils.isEmpty(imagePath)) {
